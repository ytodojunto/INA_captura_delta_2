...
def main():
    ...
    (OUT_DIR / "_debug_todas_corridas.json").write_text(json.dumps({
        "url_pedida": url,
        "cantidad_total": len(corridas_todas),
        "primeras_10": corridas_todas[:10],
        "sitecodes_presentes": sorted(set(c.get("sitecode") for c in corridas_todas)),
    }, indent=2, ensure_ascii=False))

    # diagnostico: si la ventana de 60 dias dio 0, probar ventanas mas
    # cortas y variantes de formato para encontrar donde esta el limite
    if len(corridas_todas) == 0:
        print("   [diagnostico] 0 corridas con la ventana completa, probando variantes...")
        pruebas_diag = {}
        variantes = [
            ("7d_fecha", (hoy - timedelta(days=7)).isoformat(), hoy.isoformat()),
            ("30d_fecha", (hoy - timedelta(days=30)).isoformat(), hoy.isoformat()),
            ("1d_fecha", (hoy - timedelta(days=1)).isoformat(), hoy.isoformat()),
            ("7d_datetime", (datetime.utcnow() - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%S"),
             datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S")),
            ("all_1_en_vez_de_true", None, None),  # caso especial abajo
        ]
        for nombre, ts_ini, ts_fin in variantes:
            if nombre == "all_1_en_vez_de_true":
                params = {"all": "1",
                          "forecastTimeStart": (hoy - timedelta(days=7)).isoformat(),
                          "forecastTimeEnd": hoy.isoformat(), "format": "json"}
            else:
                params = {"all": "true", "forecastTimeStart": ts_ini,
                          "forecastTimeEnd": ts_fin, "format": "json"}
            d, u, e = get_json("seriesProno", params)
            n = len(d.get("data", [])) if d else None
            pruebas_diag[nombre] = {"url": u, "cantidad": n, "error": e}
            print(f"     {nombre}: cantidad={n} url={u}")
            time.sleep(1)
        (OUT_DIR / "_debug_variantes.json").write_text(
            json.dumps(pruebas_diag, indent=2, ensure_ascii=False))
    ...
    for sitecode, slug in ESTACIONES.items():
        ...
        if not propias:
            resultado["estaciones"][slug] = {
                ...
                "nota": "0 corridas de esta estacion en la ventana all=true",
            }
            continue
        ...
        if not obs:
            ...
            resultado["estaciones"][slug] = {
                "sitecode": sitecode, "n_comparaciones": 0,
                "n_corridas_disponibles": len(propias),
                "nota": "CSV observado vacio o inexistente en data/series/",
            }
            continue
        ...
        if comparaciones:
            ...
            resultado["estaciones"][slug] = {
                ...
                "n_comparaciones": 0,
                "n_corridas_disponibles": len(propias),
                "n_corridas_elegidas": len(elegidas),
                "n_observaciones_locales": len(obs),
                "nota": "0 comparaciones - ver n_corridas_elegidas y n_observaciones_locales para diagnosticar",
            }
